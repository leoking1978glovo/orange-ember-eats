...
const CHAT_AGENT_REGION = 'f1db6c'; // ← REGIÓN CORREGIDA
...
interface ChatWidgetPanelProps {
  className?: string;
  chatKey?: number; // ← AÑADIDO: para recibir key={chatKey} del padre
...
const ChatWidgetPanel: React.FC<ChatWidgetPanelProps> = ({
  className = '',
  chatKey, // ← AÑADIDO (no se usa directamente, pero evita error de TypeScript)
  pendingMessage,
      ...
      try {
        ...
        if (!key) {
          ...
          localStorage.setItem(
            CHAT_STORAGE_KEY,
            JSON.stringify({ embedKey, conversationPrefix: taskPrefix }),
          );
        }
  ...
  return (
    ...
    >
      ...
      >
        {isLoading && (
          <p className="text-sm text-[#666666] text-center py-8">
            Conectando con {CHAT_AGENT_NAME}...
          </p>
        )}

        {error && (
          <p className="text-sm text-[#ef233c] text-center py-4 px-3 rounded-lg bg-[#f5f5f5]">
            {error}
          </p>
        )}
        ...
        {!isLoading && !error && messages.length === 0 && !pendingMessage && (
          <p className="text-sm text-[#666666] text-center py-8">
            ¡Hola! Soy {CHAT_AGENT_NAME} 👋 ¿Te ayudo con tu pedido?
          </p>
        )}
            ...
            >
              <div
                className={`max-w-[95%] text-sm leading-relaxed ${
                  isUser
                    ? 'bg-[#5F7A3A] text-white rounded-2xl rounded-br-sm px-4 py-2.5'
                    : 'bg-transparent text-[#1a1a1a] px-0 py-1'
                }`}
                style={!isUser ? { whiteSpace: 'pre-line', wordBreak: 'keep-all' } : undefined}
              >
                {text}
              </div>
            </div>
          ...
          <div className="flex justify-start">
            <div className="bg-transparent text-[#666666] rounded-none px-0 py-1 text-sm">
              <span className="inline-flex gap-1 items-center">
                {CHAT_AGENT_NAME} está escribiendo
      ...
      <form
        ...
        className="flex items-center gap-2 px-3 py-3 border-t border-[#e5e5e5] bg-white"
      >
        <input
          ref={inputRef}
          type="text"
          ...
          className="flex-1 rounded-lg border border-[#d4d4d4] bg-[#f5f5f5] px-4 py-2.5 text-sm text-[#1a1a1a] placeholder:text-[#a3a3a3] focus:outline-none focus:ring-2 focus:ring-[#5F7A3A] focus:border-[#5F7A3A] disabled:opacity-50"
          aria-label="Mensaje para el asistente"
        />
        <button
          type="submit"
          disabled={isLoading || isTyping || !!error || !inputValue.trim()}
          className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-[#5F7A3A] text-white transition-colors hover:bg-[#4a602e] disabled:opacity-40 disabled:hover:bg-[#5F7A3A]"
